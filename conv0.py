...
class LinClassifier(nn.Module):  
    ...
    # per batch sample, the Conv1d layer takes the embedding of each word 
    # and applies the convolution op to kernel-size windows
    # returning a filter-sized vector for each sliding window
    # apply max-pooling to the whole sample and input to linear

    def forward(self, batch_input):
        #print('b', batch_input.shape) # [batch_len, seqlen]
        e = self.emb(batch_input) # [batch_len, batch_seqlen, emb_size]
        e = torch.transpose(e, 1, 2) # [batch_size, emb_size, batch_seqlen]
        #print('e0', e.shape) 
        
        e = self.conv1(e) # [batch_size, emb_size, batch_len-kernel_size+1]
        #print('e1', e.shape)
...
for epoch in range(1):
    for batch in train_iter:
        model.zero_grad()
        batch_emb_matrix = torch.transpose(torch.LongTensor(batch.text), 0, 1)
        target = batch.label
